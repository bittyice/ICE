...
import { PurchaseReturnOrderApi, PurchaseReturnOrderEntity, purchaseReturnOrderSlice, supplierSlice, IceStateType, LabelValues, enums, paymentMethodSlice, PaymentMethodEntity } from 'ice-core';
...
class PurchaseReturnOrder extends React.Component<Props> {
        ...
        render: (val, row) => {
            return <ActionList>
                <Button size='small' type='link' icon={<EditOutlined />}
                    disabled={row.status != enums.OMSPurchaseReturnOrderStatus.PendingReview}
                    onClick={() => {
                        this.setState({
                            showEdit: true,
                            row: row
                ...
                >编辑</Button>
                {
                    row.status == enums.OMSPurchaseReturnOrderStatus.PendingReview ?
                        ...
                        : row.status == enums.OMSPurchaseReturnOrderStatus.Returning ?
                            ...
                            >完成</Button>
                            : row.status == enums.OMSPurchaseReturnOrderStatus.Completed ?
                                <Button size='small' type='link' ghost
                                ...
                                >结算</Button>
                                : row.status == enums.OMSPurchaseReturnOrderStatus.Invalid ?
    ...
    fetchInvalids = async () => {
        ...
        if (this.state.selectRows.some(e =>
            e.status !== enums.OMSPurchaseReturnOrderStatus.PendingReview
            && e.status !== enums.OMSPurchaseReturnOrderStatus.Returning)) {
            message.error('只能作废待审核和退货中的订单');
            return;
        }

        for (let item of this.state.selectRows) {
            await await PurchaseReturnOrderApi.invalid(item.id!);
        }
    ...
    fetchSetPaymentMethod = async (paymentMethodId: string | null) => {
        ...
        for (let item of this.state.selectRows) {
            await PurchaseReturnOrderApi.setPaymentMethod({
                id: item.id!,
