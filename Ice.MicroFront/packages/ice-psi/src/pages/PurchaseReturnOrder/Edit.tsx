...
import { consts, IceStateType, PurchaseReturnOrderApi, PurchaseReturnOrderEntity, ProductInfoHelper, QuoteApi } from 'ice-core';
        ...
        >
            <div>
                <CardEX title='基本信息' bodyStyle={{ justifyContent: 'flex-start' }}>
                    <LabelEX isMust text={'订单号'} style={{ width: '32%' }} tagStyle={{ width: 70 }}>
                        {this.props.entity?.id ? this.state.entity.orderNumber : '提交后生成'}
                    </LabelEX>
                    <LabelEX isMust text={'供应商'} style={{ width: '32%' }} tagStyle={{ width: 70 }}>
                        <Select
                            placeholder='供应商'
                            style={{ width: '100%' }}
                    ...
                    <LabelEX text={'退货总额'} style={{ width: '32%' }} tagStyle={{ width: 70 }}>
                        <InputNumber
                            style={{ width: '100%' }}
                            placeholder='退货总额'
...
export const Edit = OpenNewKey((props: Props) => {
    ...
    const onSubmit = async (entity: any) => {
        await PurchaseReturnOrderApi.update(entity);
        message.success('成功');
        props.onOk();
    }
...
export const Add = OpenNewKey((props: Props) => {
    const suppliers = useSelector((state: IceStateType) => state.supplier.allDatas) || [];
    const onSubmit = async (entity: any) => {
        await PurchaseReturnOrderApi.create(entity);
        message.success('成功');
        props.onOk();
    }
