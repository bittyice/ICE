...
const Contact = (props: {
    ...
    return <div className='p-2'>
        <div className='text-lg font-semibold'>嘿，我们希望获取你的联系方式！</div>
        <div className='w-72 flex flex-col gap-4 mt-4'>
            <Input
                placeholder='你的称呼'
            ...
            <Input
                placeholder='联系方式'
                status={(vailfail && !phone) ? 'error' : undefined}
                maxLength={consts.MinTextLength}
                value={phone}
            ...
            />
            <Button loading={loading} block type='primary' onClick={submit}>提交</Button>
        </div>
    </div>
}
...
export default class extends React.Component<{
    ...
    init = async () => {
        const qakey = Tool.getUrlVariable(window.location.search, 'guestkey') || this.props.guestkey;
        if (!qakey) {
            antMessage.error('无效的访客key');
            return;
        }
    ...
    setClientGuideQuestionText(gpt: GptEntity) {
        ...
        newMessages.push({
            role: 'system',
            content: <div className='flex flex-col gap-2 p-2'>
                <div>快速提问</div>
                {
