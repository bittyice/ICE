...
import { QaTagApi, QaTagEntity, qaTagSlice } from 'ice-core';
...
class QaTag extends React.Component<Props> {
    pageRef: CommonPageRefType | null = null;

    state = {
        // 选择的数据行
        selectRows: [] as Array<QaTagEntity>,
        // 显示添加模特框
        showAdd: false,
        // 显示编辑模块框
        showEdit: false,
...
export default () => {
    ...
    const clearReduxDatas = async () => {
        await dispatch(qaTagSlice.actions.clearAllDatas({}) as any);
    }
    
    return <QaTag clearReduxDatas={clearReduxDatas} />
}