...
using KD100SDK.Common.Request.BorderOfficial;
using KD100SDK.Common.Request.internationalshipment;
using KD100SDK.Common.Request.cloud;
...
using KD100SDK.Common.Request.Corder;
...
class Demo
{
    ...
    static void Main(string[] args)
    {
        ...
        // 返回后将该链接返回前端
        // 前端访问该链接，完成授权
        // 授权完成后，快递100推送shopId给我们，我们需要保存起来

        // 拉取订单
        // 从 Delivery100Shop 去出店铺Id
        testThirdPlatformCommitTask();
        // 调用该接口后快递100将向我们callbackUrl推送订单信息
        // 将订单信息保存起来

        // 当我们的订单发货时，调用如下接口，将第三方订单修改为已发货
        testThirdPlatformUploadNum();
    }
    ...
    static void testLabelCancel()
    {
        ...
        var timestamp = DateUtils.GetTimestamp();
        var baseParam = new CancelPrintParam()
        {
            partnerId = "",
            partnerKey = "",
            net = "",
